...
from app.schemas.reservation import GuestCreate, GuestUpdate, ReservationCreate


def create_guest(db: Session, tenant_id: UUID, payload: GuestCreate) -> Guest:
...
def update_guest(db: Session, tenant_id: UUID, guest_id: UUID, payload: GuestUpdate) -> Guest:
    guest = db.scalar(select(Guest).where(Guest.id == guest_id, Guest.tenant_id == tenant_id))
    if guest is None:
...
def cancel_reservation(db: Session, tenant_id: UUID, reservation_id: UUID) -> Reservation:
    reservation = db.scalar(select(Reservation).where(Reservation.id == reservation_id, Reservation.tenant_id == tenant_id))
    if reservation is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Reservation not found")
    ...
    if reservation.room_id:
        room = db.get(Room, reservation.room_id)
        if room and room.tenant_id == tenant_id and room.status == RoomStatus.OCCUPIED:
            room.status = RoomStatus.AVAILABLE
    db.commit()
    db.refresh(reservation)
    return reservation
